...
class Helper
{
    ...
    */
    ...
    {
        $videos = Helper::getAdminVideos();
        $tutorial = $videos[$videoType]??[];
        return Helper::getVideos($tutorial);
    }
    ...
    public static function fileNameAccordingDate($fileName, $count, $dateFormate)
    {
        $strrchr = strrchr($fileName, '_');
        if ($strrchr) {
            $fileTime = explode('_', $fileName);
        } else {
            $fileTime = explode('.', $fileName);
        }
        $datetime = new DateTime("@$fileTime[0]");
        $date_time_format = $datetime->format($dateFormate);
        $time_zone_from = 'UTC';
        ...
        $docName = 'Doc ' . $count + 1 . ' | ' . $display_date->format($dateFormate);
        return $docName;
    }
